...
if __name__ == '__main__':
    ...
    fd  = lambda x: np.array([-2*(1 - x[0]) - 2*x[0] * 2*100 * (x[1] - x[0]**2), 2 * 100 * (x[1] - x[0]**2)]) # TODO: Fill the gradient vector as [df / dx[0], df / dx[1]]
    ...
    for i in range(max_iter):
        ...
        xp = x
        x = xp - learn_rate * fd(x) # TODO: Implement your gradient descent
        gd_xs.append(x)
